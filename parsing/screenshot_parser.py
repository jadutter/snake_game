...
class ScreenshotArgumentParser(argparse.ArgumentParser):
    """
    Initialize a game of snake with the following arguments.
    """
    def __init__(self, 
            prog="SnakeGame",
            description="Read a database of recorded game states, and save them as gifs.",
            *arg, 
            **kwargs):
        super(ScreenshotArgumentParser, self).__init__(prog=prog,*arg, **kwargs)
        ...
        self.add_argument("--screenshot_path", "-sp",
                            type=str,
                            default="./screenshots/",
                            help="Set the verbosity to not print anything.")
    ...
    def parse_known_args(self, args=None, namespace=None):
        ...
        if args is not None and len(args) > 0:
            namespace, args = super(ScreenshotArgumentParser, self).parse_known_args(args=args)
        else:
            namespace, args = super(ScreenshotArgumentParser, self).parse_known_args()
